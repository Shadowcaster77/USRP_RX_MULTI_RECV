...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    for (size_t ii = 0; ii < channel_nums.size(); ii++){
        std::cout<<channel_nums[ii]<<std::endl;
    }
...
//    const size_t samps_per_buff = rx_stream->get_max_num_samps();
    ...
    MultiDeviceBufferType MultiDeviceBuffer(num_rx_channels,  std::vector<std::complex<float> >(total_num_samps) );

    // verify capacity and size
    for (unsigned int i = 0; i < num_rx_channels; ++i)
    {
      UHD_ASSERT_THROW(MultiDeviceBuffer.at(i).size() == MultiDeviceBuffer.at(i).capacity());
    }

    typedef std::vector<std::complex<float> *> BufferPointerType;

    unsigned int num_buff_ptrs = ceil((float)total_num_samps / (float)(samps_per_buff ));
    DBG_OUT( num_buff_ptrs );

    std::vector<BufferPointerType> MultiDeviceBufferPtrs;
    for (size_t j = 0; j < num_buff_ptrs; j++)
    {
      BufferPointerType bptr; // each bptr will hold samps_per_buff amount of samples per channel
      for (size_t ch = 0; ch < num_rx_channels; ch++)
	bptr.push_back( MultiDeviceBuffer.at(ch).data()+(samps_per_buff*j) );
      MultiDeviceBufferPtrs.push_back(bptr);
    }

    UHD_ASSERT_THROW(MultiDeviceBufferPtrs.size() == num_buff_ptrs);

    //the first call to recv() will block this many seconds before receiving
    double timeout = seconds_in_future + 0.1; //timeout (delay before receive + padding)

    unsigned int mdbp_idx = 0;
